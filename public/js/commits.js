...
function getCommits(user = "ShadOoW", repository = "Python-Antispam-CAPTCHA") {
  var commit, commits, commitsNode, date, dom, email, formated_date, i, message, name, node, url;
  ...
  while (i < 5) {
    ...
    formated_date = datetime.strptime(date.slice(0, -6), "%Y-%m-%dT%H:%M:%S");
    i += 1;
    node += 2;
    commit = {
      "name": name,
...
function getRepInfo(repository = "Python-Antispam-CAPTCHA") {
  var author, date, description, dom, formated_date, language, name, repInfo, repositoryNode, url, watchers;
